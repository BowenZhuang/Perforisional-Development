...
prop.table(table(risk2009$female))
...
risk2009 %>%
  ...
  summarise(meanBMI = mean(bmipct),
            sdBMI = median(bmipct), count=n(),.groups = "drop")
```
...
```{r, 6model2int}
model2 <- glm(lose.wt.01 ~ female+bmipct+female:bmipct, family = binomial, 
              data = risk2009)
coef(summary(model2))
cat(" Residual deviance = ", 
...
```{r, message = NA}
model3 <- glm(lose.wt.01 ~ female+bmipct, family = binomial, 
              data = risk2009)
coef(summary(model3))
cat(" Residual deviance = ", 
...
model4 <- glm(lose.wt.01 ~ female+bmipct+sport, family = binomial, 
              data = risk2009)
coef(summary(model4))
cat(" Residual deviance = ", 
